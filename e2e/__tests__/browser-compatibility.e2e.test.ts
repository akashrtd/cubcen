...
browsers.forEach(({ name, ...device }) => {
  test.describe(`Browser Compatibility - ${name}`, () => {
    ...
    test(`navigation works on ${name}`, async ({ page }) => {
      ...
      const pages = [
        { link: '[data-testid="nav-analytics"]', url: '/dashboard/analytics', title: 'Analytics' },
        { link: '[data-testid="nav-platforms"]', url: '/dashboard/platforms', title: 'Platform Management' },
        { link: '[data-testid="nav-users"]', url: '/dashboard/users', title: 'User Management' },
        { link: '[data-testid="nav-settings"]', url: '/dashboard/settings', title: 'Settings' },
      ]
    ...
    test(`forms work correctly on ${name}`, async ({ page }) => {
      ...
      await page.fill('[data-testid="platform-url"]', 'https://test.example.com')
    ...
    test(`charts and visualizations render on ${name}`, async ({ page }) => {
      ...
      const chartElements = await page.locator('[data-testid="task-status-chart"] svg').count()
      ...
      if (!name.includes('Mobile')) {
        ...
        await page.hover('[data-testid="task-status-chart"] .recharts-pie-sector')
    ...
    test(`local storage and session management work on ${name}`, async ({ page }) => {
