...
import { CircuitBreaker, CircuitBreakerError, createPlatformCircuitBreaker } from '../circuit-breaker';
...
describe('CircuitBreaker', () => {
  ...
  describe('expectedErrors option', () => {
    it('should only count expected errors as failures', async () => {
      ...
      await expect(cb.execute(() => Promise.reject(unexpectedError))).rejects.toThrow('network failure');
      expect(cb.getStats().failures).toBe(0);
      expect(cb.getStats().state).toBe('closed');
      
      // Expected errors should count as failures
      await expect(cb.execute(() => Promise.reject(expectedError))).rejects.toThrow('expected failure');
      await expect(cb.execute(() => Promise.reject(expectedError))).rejects.toThrow('expected failure');
...
describe('createPlatformCircuitBreaker', () => {
  ...
  it('should handle network-related errors as expected failures', async () => {
    ...
    await expect(cb.execute(() => Promise.reject(networkError))).rejects.toThrow('ECONNREFUSED');
