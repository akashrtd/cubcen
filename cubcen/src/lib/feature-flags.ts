...
export class FeatureFlagService {
   ...
   */
  public getStatusSummary(): Record<string, { enabled: boolean; source: 'config' | 'override' }> {
    const summary: Record<string, { enabled: boolean; source: 'config' | 'override' }> = {};
    ...
    for (const [key, value] of Object.entries(allFlags)) {
      summary[key] = {
        enabled: value,
        source: this.overrides[key as keyof FeatureFlags] !== undefined ? 'override' : 'config'
      };
    }
 ...
 */
export function applyRolloutStage(stage: keyof typeof FEATURE_ROLLOUT_STAGES): void {
