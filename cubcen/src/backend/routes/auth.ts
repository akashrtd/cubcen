...
router.get('/me', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get user info failed', error as Error, { userId: req.user?.id })
...
router.get('/users', authenticate, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get all users failed', error as Error, { requestedBy: req.user?.id })
 ...
 */
router.get('/users/:id', authenticate, requireSelfOrAdmin('id'), validateParams(idParamSchema), async (req: Request, res: Response) => {
  try {
    const { id } = req.params
    
    logger.info('Get user by ID request', { userId: id, requestedBy: req.user!.id })
 ...
 */
router.put('/users/:id/role', authenticate, requireAdmin, validateParams(idParamSchema), validateBody(updateUserRoleSchema), async (req: Request, res: Response) => {
  try {
    const { id } = req.params
    const { role } = req.body
 ...
 */
router.delete('/users/:id', authenticate, requireAdmin, validateParams(idParamSchema), async (req: Request, res: Response) => {
  try {
    const { id } = req.params
