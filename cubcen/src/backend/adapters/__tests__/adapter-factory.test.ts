...
import { AdapterFactory, AdapterManager, AdapterConstructor } from '../adapter-factory';
...
describe('AdapterFactory', () => {
  ...
  describe('registerAdapter', () => {
    ...
    it('should allow overriding existing adapter types', () => {
      AdapterFactory.registerAdapter('n8n', TestAdapter);
      AdapterFactory.registerAdapter('n8n', MockPlatformAdapter as AdapterConstructor);
  ...
  describe('removeAdapter', () => {
    ...
    it('should handle removal of non-existent adapter', async () => {
      await expect(AdapterFactory.removeAdapter('non-existent')).resolves.not.toThrow();
    });

    it('should handle disconnect errors gracefully', async () => {
      const adapter = AdapterFactory.createAdapter(testConfig);
      jest.spyOn(adapter, 'disconnect').mockRejectedValue(new Error('Disconnect failed'));
  ...
  describe('isAdapterRegistered', () => {
    ...
    it('should return false for unregistered adapter types', () => {
      expect(AdapterFactory.isAdapterRegistered('unknown' as PlatformType)).toBe(false);
    });
  });
...
describe('AdapterManager', () => {
  ...
  describe('addPlatform', () => {
    ...
    it('should remove adapter if connection fails', async () => {
      ...
      await expect(manager.addPlatform(testConfig)).rejects.toThrow('Connection failed');
  ...
  describe('removePlatform', () => {
    ...
    it('should handle removal of non-existent platform', async () => {
      await expect(manager.removePlatform('non-existent')).resolves.not.toThrow();
    });
  });
  ...
  describe('getAdaptersByType', () => {
    it('should return adapters filtered by type', async () => {
      ...
      const makeConfig = { ...testConfig, id: 'make-platform', type: 'make' as PlatformType };
  ...
  describe('disconnectAll', () => {
    ...
    it('should handle disconnect errors gracefully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Error disconnecting adapter:', expect.any(Error));
      expect(manager.getAllAdapters()).toHaveLength(0);
