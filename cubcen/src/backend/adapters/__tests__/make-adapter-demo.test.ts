...
describe('Make.com Platform Adapter Demo', () => {
  ...
  describe('Real-world usage scenarios', () => {
    it('should demonstrate complete Make.com integration workflow', async () => {
      ...
      console.log(`   Connected at: ${connectionResult.lastConnected?.toISOString()}`);
      ...
      for (const agent of agents) {
        ...
        if (status.metrics) {
          ...
          console.log(`     - Avg Execution Time: ${status.metrics.averageExecutionTime}ms`);
          console.log(`     - Error Rate: ${(status.metrics.errorRate * 100).toFixed(1)}%`);
        }
      }
      ...
      if (activeAgent) {
        ...
        const executionResult = await adapter.executeAgent(activeAgent.id, executionParams);
        
        console.log(`   Execution Result:`);
        console.log(`     - Success: ${executionResult.success}`);
        console.log(`     - Execution Time: ${executionResult.executionTime}ms`);
        console.log(`     - Timestamp: ${executionResult.timestamp.toISOString()}`);
    ...
    it('should demonstrate OAuth authentication flow', async () => {
      ...
      expect(agents.length).toBeGreaterThan(0);
      console.log(`✅ Successfully retrieved ${agents.length} scenarios with OAuth`);
    ...
    it('should demonstrate performance characteristics', async () => {
      ...
      console.log(`✅ Completed 5 sequential health checks in ${sequentialDuration}ms`);
      console.log(`   Average: ${(sequentialDuration / 5).toFixed(1)}ms per request`);
