...
export class WebSocketService {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('auth:authenticate', async (token: string, callback) => {
        try {
          ...
          logger.error('Socket authentication error', error as Error, { socketId: socket.id })
          if (typeof callback === 'function') {
  ...
  private async authenticateSocket(socket: any, token: string): Promise<AuthResult> { // eslint-disable-line @typescript-eslint/no-explicit-any
    try {
  ...
  private handleAgentSubscription(socket: any, agentIds: string[]): void { // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    if (agentIds.length === 0) {
      ...
      if (socket.data.userRole === 'admin' || socket.data.userRole === 'operator') {
        ...
        logger.debug('Socket subscribed to all agents', { socketId: socket.id, userId })
      }
    } else {
  ...
  private handleTaskSubscription(socket: any, taskIds: string[]): void { // eslint-disable-line @typescript-eslint/no-explicit-any
    const userId = socket.data.userId
    ...
    if (taskIds.length === 0) {
      ...
      logger.debug('Socket subscribed to all tasks', { socketId: socket.id, userId })
    } else {
  ...
  private handlePlatformSubscription(socket: any, platformIds: string[]): void { // eslint-disable-line @typescript-eslint/no-explicit-any
    const userId = socket.data.userId
    ...
    if (platformIds.length === 0) {
      ...
      logger.debug('Socket subscribed to all platforms', { socketId: socket.id, userId })
    } else {
  ...
  private handleAgentUnsubscription(socket: any, agentIds: string[]): void { // eslint-disable-line @typescript-eslint/no-explicit-any
    if (agentIds.length === 0) {
  ...
  private handleTaskUnsubscription(socket: any, taskIds: string[]): void { // eslint-disable-line @typescript-eslint/no-explicit-any
    if (taskIds.length === 0) {
  ...
  private handlePlatformUnsubscription(socket: any, platformIds: string[]): void { // eslint-disable-line @typescript-eslint/no-explicit-any
    if (platformIds.length === 0) {
  ...
  private handleDisconnection(socket: any, reason: string): void { // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  private cleanupStaleConnections(): void {
    ...
    for (const [socketId, connection] of this.connections.entries()) {
      if (now.getTime() - connection.lastActivity.getTime() > staleThreshold) {
        logger.info('Cleaning up stale connection', { socketId, userId: connection.userId })
   ...
   */
  public notifyAgentStatusChange(agentId: string, status: 'active' | 'inactive' | 'error' | 'maintenance', metadata?: Record<string, unknown>): void {
   ...
   */
  public notifyAgentHealthChange(agentId: string, health: {
   ...
   */
  public notifyAgentConnection(agentId: string, platformId: string, connected: boolean, reason?: string): void {
    if (connected) {
      ...
      logger.debug('Broadcasted agent disconnected event', { agentId, platformId, reason })
    }
  }
   ...
   */
  public notifyTaskCreated(taskId: string, agentId: string, type: string, priority: 'low' | 'medium' | 'high' | 'critical', scheduledAt: Date, createdBy: string): void {
  ...
  public notifyTaskStarted(taskId: string, agentId: string, startedAt: Date, estimatedDuration?: number): void {
  ...
  public notifyTaskCompleted(taskId: string, agentId: string, completedAt: Date, duration: number, success: boolean, result?: Record<string, unknown>): void {
    ...
    logger.debug('Broadcasted task completed event', { taskId, agentId, success })
  }

  public notifyTaskFailed(taskId: string, agentId: string, failedAt: Date, error: { code: string; message: string; details?: Record<string, unknown> }, retryCount: number, willRetry: boolean): void {
    ...
    logger.debug('Broadcasted task failed event', { taskId, agentId, errorCode: error.code })
  }

  public notifyTaskCancelled(taskId: string, agentId: string, cancelledAt: Date, cancelledBy: string, reason: string): void {
    ...
    logger.debug('Broadcasted task cancelled event', { taskId, agentId, reason })
  }

  /**
   * Notify about task status change (called by TaskService)
   */
  public notifyTaskStatusChange(taskId: string, status: TaskStatus, metadata?: Record<string, unknown>): void {
    ...
    switch (status) {
      case 'PENDING':
        ...
        break
      case 'RUNNING':
        this.notifyTaskStarted(taskId, metadata?.agentId as string, metadata?.startedAt as Date || new Date())
        break
      case 'COMPLETED':
   ...
   */
  public notifyTaskProgress(taskId: string, progress: number, message?: string): void {
    ...
    logger.debug('Broadcasted task progress event', { taskId, progress, message })
  }

  /**
   * Notify about task error (called by TaskService)
   */
  public notifyTaskError(taskId: string, error: string, context?: Record<string, unknown>): void {
   ...
   */
  public notifyWorkflowStatusChange(executionId: string, status: string, metadata?: Record<string, unknown>): void {
    const event = {
      executionId,
      status,
      metadata,
    ...
    logger.debug('Broadcasted workflow status change event', { executionId, status })
  }
  ...
  public notifyWorkflowProgress(executionId: string, progress: any): void {
    ...
    logger.debug('Broadcasted workflow progress event', { executionId, progress: progress.progress })
  }

  /**
   * Notify about workflow error (called by WorkflowService)
   */
  public notifyWorkflowError(executionId: string, error: string, context?: Record<string, unknown>): void {
    const event = {
      executionId,
      error: {
        code: 'WORKFLOW_ERROR',
...
export function initializeWebSocketService(httpServer: HTTPServer, agentService?: AgentService): WebSocketService {
  if (!webSocketService) {
