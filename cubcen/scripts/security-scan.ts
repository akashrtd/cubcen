...
class SecurityScanner {
  ...
  private async testSQLInjection(): Promise<TestResult> {
    ...
    try {
      for (const payload of sqlPayloads) {
        const response = await axios.get(`${this.baseUrl}/api/cubcen/v1/users`, {
        ...
        if (response.data && typeof response.data === 'string') {
          const errorIndicators = ['sql', 'syntax error', 'mysql', 'postgresql', 'sqlite']
          const responseStr = response.data.toLowerCase()
          
          if (errorIndicators.some(indicator => responseStr.includes(indicator))) {
            return {
              passed: false,
        ...
        if (response.status === 200 && response.data && Array.isArray(response.data)) {
          ...
          if (response.data.length > 100) { // Suspiciously large result set
            return {
              passed: false,
              message: 'Possible SQL injection - unexpected large dataset returned',
  ...
  private async testCSRFProtection(): Promise<TestResult> {
    try {
      ...
      if (response.status === 200) {
        return {
          passed: false,
          message: 'CSRF protection may be missing - state-changing request succeeded without token',
  ...
  private async testSecurityHeaders(): Promise<TestResult> {
    try {
      ...
      const incorrectHeaders: Array<{ header: string; expected: string; actual: string }> = []
      ...
      for (const [header, expectedValue] of Object.entries(requiredHeaders)) {
        ...
        if (!actualValue) {
          ...
          incorrectHeaders.push({ header, expected: expectedValue, actual: actualValue })
        }
      }
      ...
      if (missingHeaders.length > 0 || incorrectHeaders.length > 0 || exposedInfo.length > 0) {
        return {
          passed: false,
  ...
  private async testRateLimiting(): Promise<TestResult> {
    try {
      ...
      if (rateLimitedResponses.length === 0) {
        return {
          passed: false,
          message: 'Rate limiting may not be implemented - no 429 responses received',
      ...
      return {
        ...
        details: { totalRequests: requests.length, rateLimitedCount: rateLimitedResponses.length },
        severity: 'low'
      }
    } catch (error) {
      if (axios.isAxiosError(error) && error.response?.status === 404) {
  ...
  private async testAuthenticationSecurity(): Promise<TestResult> {
    ...
    try {
      ...
      const malformedTokenResponse = await axios.get(`${this.baseUrl}/api/cubcen/v1/users`, {
    ...
    try {
      ...
      if (setCookieHeader) {
        ...
        const sessionCookies = cookies.filter(cookie => 
          cookie.toLowerCase().includes('session') || cookie.toLowerCase().includes('connect.sid')
        )
  ...
  private async testInputValidation(): Promise<TestResult> {
    const invalidInputs = [
      ...
      { field: 'id', value: '../../etc/passwd', expected: 'path traversal protection' }
    ]
  ...
  private async testFileUploadSecurity(): Promise<TestResult> {
    try {
      ...
      const dangerousFiles = [
        { name: 'malware.exe', content: 'fake executable', type: 'application/octet-stream' },
      ...
      for (const file of dangerousFiles) {
        ...
        const response = await axios.post(`${this.baseUrl}/api/cubcen/v1/upload`, formData, {
  ...
  private async testInformationDisclosure(): Promise<TestResult> {
    ...
    const disclosedInfo: Array<{ endpoint: string; status: number; info: string }> = []
  ...
  private generateReport(): void {
    ...
    if (failed > 0) {
      ...
      if (critical.length > 0) {
        ...
        critical.forEach(test => console.log(`      - ${test.testName}: ${test.message}`))
      }

      if (high.length > 0) {
        console.log(`\n   🟠 HIGH (${high.length}):`)
        high.forEach(test => console.log(`      - ${test.testName}: ${test.message}`))
      }

      if (medium.length > 0) {
        console.log(`\n   🟡 MEDIUM (${medium.length}):`)
        medium.forEach(test => console.log(`      - ${test.testName}: ${test.message}`))
      }

      if (low.length > 0) {
        console.log(`\n   🔵 LOW (${low.length}):`)
        low.forEach(test => console.log(`      - ${test.testName}: ${test.message}`))
      }
    }
    ...
    if (failed === 0) {
      ...
      console.log(`   🔧 Address the failed tests above, prioritizing critical and high severity issues.`)
      console.log(`   📚 Review the security best practices guide for detailed remediation steps.`)
    ...
    const criticalOrHigh = this.results.filter(r => 
