...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const stats = {
        ...
        inactive: fetchedUsers.filter((u: User) => u.status === 'inactive')
          .length,
        suspended: fetchedUsers.filter((u: User) => u.status === 'suspended')
          .length,
      }
      setUserStats(stats)
    } catch (error) {
  ...
  const handleUserDeactivate = async (user: User) => {
    ...
    if (
      confirm(
        `Are you sure you want to ${action} ${user.name}? This action will be logged for audit purposes.`
      )
    ) {
      try {
        setLoading(true)
        ...
        toast.success(
          `${user.name} has been ${action}ed successfully. Action logged for audit trail.`
        )
      } catch (error) {
  ...
  const handleFormSubmit = async (data: any) => {
    try {
      ...
      if (formMode === 'invite') {
        toast.success(
          `Invitation sent to ${data.email}. Action logged for audit trail.`
        )
      } else if (formMode === 'create') {
        toast.success(
          `User ${data.name} created successfully. Action logged for audit trail.`
        )
        ...
        if (savedUser) {
          ...
          setUserStats(prevStats => ({
            ...
            inactive:
              prevStats.inactive + (savedUser.status === 'inactive' ? 1 : 0),
            suspended:
              prevStats.suspended + (savedUser.status === 'suspended' ? 1 : 0),
          }))
        }
      } else {
        toast.success(
          `User ${data.name} updated successfully. Action logged for audit trail.`
        )
      ...
      >
        <div className="container mx-auto py-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-muted-foreground">
                Manage user accounts, roles, and permissions with full audit
                trail logging
              </p>
            </div>
          ...
          <Alert>
            ...
            <AlertDescription>
              All user management actions are logged with full audit trails for
