...
describe('ErrorPatterns', () => {
  ...
  it('fetches and displays error patterns', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/api/cubcen/v1/errors/patterns?'))
  })
  ...
  it('handles custom time range prop', async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('from=2024-01-01T00%3A00%3A00.000Z'))
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining('to=2024-01-02T00%3A00%3A00.000Z'))
    })
  })
  ...
  it('handles API errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Failed to load error patterns')).toBeInTheDocument()
    })
  })
  ...
  it('displays empty state when no patterns found', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('No recurring error patterns found in the selected time range.')).toBeInTheDocument()
    })
  })
  ...
  it('allows expanding error details', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Connection timeout error occurred')).toBeInTheDocument()
    })
  })
  ...
  it('displays error resolution suggestions', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Check network connectivity and increase timeout values')).toBeInTheDocument()
    })
  })
})