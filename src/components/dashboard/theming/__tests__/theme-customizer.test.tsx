...
describe('ThemeCustomizer', () => {
  ...
  it('triggers file input when import button is clicked', () => {
    ...
    const fileInput = screen
      .getByRole('button', { name: /import theme/i })
  ...
  it('displays footer information', async () => {
    ...
    expect(
      screen.getByText('Changes are applied automatically')
    ).toBeInTheDocument()

    // Wait for validation results
    await waitFor(() => {
      expect(
        screen.getByText(/Accessibility: \d+\/\d+ passed/)
      ).toBeInTheDocument()
    })
  })
  ...
  it('handles invalid import file gracefully', () => {
    ...
    const mockFile = new File(['invalid'], 'theme.json', {
      type: 'application/json',
    })
    Object.defineProperty(fileInput, 'files', {
      value: [mockFile],
