...
const DashboardThemeProviderContext =
  createContext<DashboardThemeProviderState>(initialState)
...
export function DashboardThemeProvider({
  ...
  const [dashboardTheme, setDashboardThemeState] = useState<DashboardTheme>(
    () => {
      return customTheme
        ? { ...defaultDashboardTheme, ...customTheme }
  ...
  const updateCSSVariables = useCallback(
    (currentTheme: 'dark' | 'light') => {
            ...
            if (typeof subValue === 'string') {
              ...
              Object.entries(subValue).forEach(([nestedKey, nestedValue]) => {
                root.style.setProperty(
                  `--dashboard-${key}-${subKey}-${nestedKey}`,
      ...
      Object.entries(themeToApply.typography.fontSize).forEach(
        ([key, value]) => {
          root.style.setProperty(`--dashboard-text-${key}`, value)
        }
      )

      Object.entries(themeToApply.typography.fontWeight).forEach(
        ([key, value]) => {
          root.style.setProperty(`--dashboard-font-${key}`, value.toString())
        }
      )

      Object.entries(themeToApply.typography.lineHeight).forEach(
        ([key, value]) => {
          root.style.setProperty(
            `--dashboard-line-height-${key}`,
            value.toString()
          )
        }
      )
      ...
      Object.entries(themeToApply.animations.duration).forEach(
        ([key, value]) => {
  ...
  const validateContrastRatio = useCallback(
  ...
  const getContrastRatioValue = useCallback(
  ...
  const handleSetDashboardTheme = useCallback(
      ...
      if (validateContrast && newTheme.colors) {
        ...
        if (colors.text) {
          Object.entries(colors.text).forEach(([key, color]) => {
            ...
            if (ratio < 4.5) {
              console.warn(
                `Dashboard theme: Text color "${key}" (${color}) does not meet WCAG AA contrast requirements against background (${background}). Ratio: ${ratio.toFixed(2)}`
              )
            }
          })
        }
        ...
        if (colors.status) {
          Object.entries(colors.status).forEach(([key, color]) => {
            const ratio = getContrastRatio(color, background)
            if (ratio < 3.0) {
              // Lower requirement for non-text elements
              console.warn(
                `Dashboard theme: Status color "${key}" (${color}) may have insufficient contrast against background (${background}). Ratio: ${ratio.toFixed(2)}`
              )
            }
          })
        }
