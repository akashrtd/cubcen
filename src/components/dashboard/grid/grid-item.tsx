...
export function GridItem({
  ...
  const getColSpanClasses = (span: number | { mobile?: number; tablet?: number; desktop?: number }) => {
  ...
  const getRowSpanClasses = (span: number | { mobile?: number; tablet?: number; desktop?: number }) => {
    if (typeof span === 'number') {
      return span > 1 ? `row-span-${Math.min(span, 6)}` : ''
    }
    
    const classes = []
    if (span.mobile && span.mobile > 1) classes.push(`row-span-${Math.min(span.mobile, 6)}`)
    if (span.tablet && span.tablet > 1) classes.push(`md:row-span-${Math.min(span.tablet, 6)}`)
    if (span.desktop && span.desktop > 1) classes.push(`lg:row-span-${Math.min(span.desktop, 6)}`)
    
    return classes.join(' ')
  }

  // Generate responsive classes for order
  const getOrderClasses = (orderValue?: number | { mobile?: number; tablet?: number; desktop?: number }) => {
    ...
    const classes = []
    if (orderValue.mobile) classes.push(`order-${Math.min(orderValue.mobile, 12)}`)
    if (orderValue.tablet) classes.push(`md:order-${Math.min(orderValue.tablet, 12)}`)
    if (orderValue.desktop) classes.push(`lg:order-${Math.min(orderValue.desktop, 12)}`)
