...
const eventTypeDescriptions: Record<NotificationEventType, string> = {
  ...
  [NotificationEventType.PLATFORM_DISCONNECTED]:
    'When a platform connection is lost',
}
...
export function NotificationPreferences({
  ...
  const updatePreference = (
    ...
    setPreferences(prev =>
      prev.map(pref =>
        pref.eventType === eventType ? { ...pref, [field]: value } : pref
      )
    )
  }
  ...
  const testNotification = async (channel: NotificationChannelType) => {
    try {
      ...
      toast.error(
        `Failed to send test notification via ${channelLabels[channel]}`
      )
