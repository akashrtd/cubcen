...
const envSchema = z.object({
  ...
  DATABASE_MAX_CONNECTIONS: z.string().transform(Number).pipe(z.number().min(1)).default('10'),
  ...
  ENABLE_KANBAN_BOARD: z.string().transform(val => val === 'true').default('true'),
  ENABLE_WORKFLOW_ORCHESTRATION: z.string().transform(val => val === 'true').default('false'),
  ENABLE_ADVANCED_AUTH: z.string().transform(val => val === 'true').default('false'),
  ...
  WEBSOCKET_HEARTBEAT_INTERVAL: z.string().transform(Number).pipe(z.number().min(1000)).default('30000'),
  
  // Health Check Configuration
  HEALTH_CHECK_INTERVAL: z.string().transform(Number).pipe(z.number().min(1000)).default('60000'),
  ...
  BACKUP_INTERVAL_HOURS: z.string().transform(Number).pipe(z.number().min(1)).default('24'),
  BACKUP_RETENTION_DAYS: z.string().transform(Number).pipe(z.number().min(1)).default('7'),
