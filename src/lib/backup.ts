...
export class BackupService {
  ...
  public async createBackup(compress: boolean = true): Promise<BackupResult> {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          'Failed to clean up failed backup file',
        ...
        if (file.endsWith('.metadata.json')) {
          try {
            ...
            try {
              ...
              logger.warn('Backup file missing for metadata', {
                file,
                path: metadata.path,
              })
            }
          } catch (error) {
            logger.warn('Failed to read backup metadata', error as Error, {
              file,
            })
          }
        }
  ...
  private async restoreSQLiteDatabase(
    backupPath: string,
    targetPath: string,
    compressed: boolean
  ): Promise<void> {
