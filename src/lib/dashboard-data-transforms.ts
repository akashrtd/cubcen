...
export class DashboardDataTransforms {
   ...
   */
  static transformDailyTaskTrends(data: AnalyticsData['dailyTaskTrends']): ChartData {
    return {
      datasets: [
        {
   ...
   */
  static transformTasksByStatus(data: AnalyticsData['tasksByStatus']): ChartData {
    return {
      datasets: [{
 ...
 /**
   ...
   */
  static transformTasksByPriority(data: AnalyticsData['tasksByPriority']): ChartData {
    return {
      datasets: [{
   ...
   */
  static transformPlatformDistribution(data: AnalyticsData['platformDistribution']): ChartData {
    return {
      datasets: [{
   ...
   */
  static transformErrorPatterns(data: AnalyticsData['errorPatterns']): ChartData {
    return {
      datasets: [{
        label: 'Error Patterns',
        data: data.map(item => ({
          x: item.error.substring(0, 30) + (item.error.length > 30 ? '...' : ''),
          y: item.count,
          label: item.error,
          metadata: { fullError: item.error }
        }))
      }],
      labels: data.map(item => item.error.substring(0, 30) + (item.error.length > 30 ? '...' : '')),
      metadata: { chartType: 'bar' }
    }
  }
   ...
   */
  static transformLegacyRechartsData(legacyData: LegacyChartData[], chartType: string): ChartData {
    if (!Array.isArray(legacyData) || legacyData.length === 0) {
      return { datasets: [], labels: [], metadata: { chartType } }
    }

    const firstItem = legacyData[0]
    const keys = Object.keys(firstItem).filter(key => typeof firstItem[key] === 'number')
