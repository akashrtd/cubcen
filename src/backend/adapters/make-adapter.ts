...
export class MakePlatformAdapter extends BasePlatformAdapter {
  ...
  async authenticate(credentials: PlatformCredentials): Promise<AuthResult> {
    try {
      ...
      throw new Error(
        'Invalid credentials provided. Requires apiToken or OAuth credentials.'
      )
    } catch (error) {
  ...
  private async refreshAccessToken(
    ...
    try {
      ...
      throw new Error(
        `Failed to refresh access token: ${this.extractErrorMessage(error)}`
      )
    }
  }
  ...
  private extractCapabilities(scenario: MakeScenario): string[] {
    ...
    if (scenario.blueprint?.flow) {
      const moduleTypes = new Set(
        scenario.blueprint.flow.map(module => module.module)
      )
