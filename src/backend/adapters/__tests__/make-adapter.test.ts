...
describe('MakePlatformAdapter', () => {
  ...
  describe('authenticate', () => {
    it('should authenticate with API token successfully', async () => {
      ...
      mockAxiosInstance.get.mockResolvedValue({ data: mockScenarios });
    ...
    it('should authenticate with OAuth access token successfully', async () => {
      ...
      expect(mockAxiosInstance.defaults.headers.common['Authorization']).toBe('Bearer access-token');
    });
    ...
    it('should handle invalid credentials', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('Authentication failed: Invalid credentials provided. Requires apiToken or OAuth credentials.');
    });
  });
      ...
      const mockScenarios = [
        {
          ...
          blueprint: {
            flow: [
              { id: 1, module: 'webhook', version: 1, parameters: {}, mapper: {}, metadata: { designer: { x: 0, y: 0 } } },
              { id: 2, module: 'http', version: 1, parameters: {}, mapper: {}, metadata: { designer: { x: 100, y: 0 } } }
            ],
            metadata: {}
          }
        },
  ...
  describe('getAgentStatus', () => {
    it('should get agent status successfully', async () => {
      ...
      mockAxiosInstance.get
        .mockResolvedValueOnce({ data: mockScenario })
        .mockResolvedValueOnce({ data: mockExecutions });
      ...
      expect(mockAxiosInstance.get).toHaveBeenCalledWith('/scenarios/1/executions', {
        params: { limit: 10 }
      });
    });
  ...
  describe('error handling', () => {
    ...
    it('should handle network errors', async () => {
      ...
      expect(result.error).toBe('Authentication failed: Network error: ECONNREFUSED');
    });
