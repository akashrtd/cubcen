...
describe('N8nPlatformAdapter', () => {
  ...
  describe('constructor', () => {
    ...
    it('should throw error for invalid configuration', () => {
      ...
      expect(() => new N8nPlatformAdapter(invalidConfig as PlatformConfig)).toThrow(
        'n8n adapter requires credentials configuration'
      );
    });
  ...
  describe('authenticate', () => {
    it('should authenticate successfully with API key', async () => {
      ...
      expect(mockAxiosInstance.defaults.headers.common['X-N8N-API-KEY']).toBe('test-api-key');
    ...
    it('should authenticate successfully with email/password', async () => {
      ...
      expect(mockAxiosInstance.defaults.headers.common['Authorization']).toBe('Bearer jwt-token');
  ...
  describe('discoverAgents', () => {
    ...
    it('should discover agents successfully', async () => {
      ...
      expect(agents[0]).toMatchObject({
        ...
        platformType: 'n8n',
        status: 'active',
        capabilities: expect.arrayContaining(['n8n-nodes-base.start', 'n8n-nodes-base.httpRequest', 'tag:automation', 'tag:test'])
      });
  ...
  describe('executeAgent', () => {
    it('should execute agent successfully', async () => {
      ...
      expect(mockAxiosInstance.post).toHaveBeenCalledWith('/workflows/workflow-1/execute', {
        data: { input: 'test' }
      });
    });

    it('should handle execution failure', async () => {
      mockAxiosInstance.post.mockRejectedValueOnce(new Error('Execution failed'));
  ...
  describe('disconnect', () => {
    it('should disconnect successfully', async () => {
      ...
      expect(mockAxiosInstance.defaults.headers.common['X-N8N-API-KEY']).toBeUndefined();
    });
  });
  ...
  describe('circuit breaker integration', () => {
    it('should use circuit breaker for API calls', async () => {
      ...
      await expect(adapter.healthCheck()).resolves.toMatchObject({ status: 'unhealthy' });
      await expect(adapter.healthCheck()).resolves.toMatchObject({ status: 'unhealthy' });
      await expect(adapter.healthCheck()).resolves.toMatchObject({ status: 'unhealthy' });
