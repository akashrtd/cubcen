 ...
 */
router.get('/', authenticate, requireAuth, validateQuery(platformQuerySchema), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get platforms failed', error as Error, { userId: req.user?.id })
 ...
 */
router.post('/', authenticate, requireAdmin, validateBody(createPlatformSchema), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Create platform failed', error as Error, { userId: req.user?.id })
 ...
 */
router.put('/:id', authenticate, requireAdmin, validateParams(idParamSchema), validateBody(updatePlatformSchema), async (req: Request, res: Response) => {
  try {
    const { id } = req.params
 ...
 */
router.post('/test-connection', authenticate, requireAdmin, validateBody(testConnectionSchema), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Platform connection test failed', error as Error, { userId: req.user?.id })
 ...
 */
router.post('/:id/sync', authenticate, requireOperator, validateParams(idParamSchema), async (req: Request, res: Response) => {
  try {
    const { id } = req.params
